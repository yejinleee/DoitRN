...
- 마운트 때
  ...
  <!-- useLayoutEffect Finished가 먼저나온다 ? ? ?   -->
...
fetch('http 주소').then((res) => ... ).catch(setError)
...
<!-- 내 이해 정리
use커스텀훅 만들 때 반환값타입 명시를 꼭 해주어야한다.
[state, setState] = use커스텀훅
이렇게 쓰일것이기 때문에 반환값도
return  [상태, 상태변경함수]
이면 된다.
원래 본문에서 const on뭐뭐 = () => ... 이런식으로 해두었던 걸
커스텀 훅에 쓰면된다. 내용은 같음.
사용할 컴포넌트에선 [.., set ...] = 만든커스텀훅(인자가필요하면여기);
이면 된다.
-->
...
- Promise <br/>
  resolve 된 값 or reject 된 오류(Error타입) 둘 중 하나 반드시 발생시킴.
  <br/>Promise<T\>는 resolve된 값의 타입을 타입 변수 T로 하는 제네릭 타입이다. 즉, T타입을 resolve 한다는 것.

- async/await<br/>

  - await 사용법 : const resolve된값 = await Promise객체

  await 연산자를 사용하려면 **이 연산자를 호출하는 함수 앞**에 반드시 async라는 함수 수정자 붙일 것.<br/>
  ...
  async 함수는 항상 Promimse 타입 객체를 반환한다.

  오류가 reject되면 cath메서드를 통해 reject된 Error 타입 오류를 수신할 수 있다.(해야한다)
